...
defensive score for up to the last 10 most recent games. If less than 10 games have been played in the current season, we still 
take the average of the previous games. Furthermore, we developed a LastTenGames column that measures how many games each team has won. 
This measures a "hot" or "cold" streak for both teams.
"""
...
"""Furthermore, we utilize the nba2k dataset to sort the values by season, team, and minutes played for each player. Afterwards, 
we had to ensure that the "SEASON" column matched with the"""
...
The below RandomForestClassifier is initialized without grid search / hyperparameter tuning to demonstrate an initial baseline for our accuracy score. 
We will see how through utilizing different classifiers, including stacking / ensemble methods as well as utilizing gridsearch for hyperparameter tuning 
will improve our classifiers from this initial baseline measurement in subsequent cells.
"""
